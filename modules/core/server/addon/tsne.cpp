...
void TSNE::run(double* image, double* word, double* word_solution, int N_I, int N_W, int D, double* Y, int no_dims, double perplexity, double theta, int max_iter) {
    ...
    // Allocate some memory
    double weight = 0.9;
    double* dYI = (double*)malloc(N_I * no_dims * sizeof(double));
    double* dY = (double*)malloc(N * no_dims * sizeof(double));
    ...
    printf("Computing input similarities...\n");
    start = clock();
    zeroMean(image, N_I, D);
    zeroMean(word, N, D);
    double max_I = .0, max_W = .0;
    for (int i = 0; i < N_I * D; i++) {
        if (image[i] > max_I) max_I = image[i];
    }
    for (int i = 0; i < N * D; i++) {
        if (word[i] > max_W) max_W = word[i];
    }
    for (int i = 0; i < N_I * D; i++) image[i] /= max_I;
    for (int i = 0; i < N * D; i++) word[i] /= max_W;

    // Compute input similarities for exact t-SNE
    double* image_P=0; int* row_image_P=0; int* col_image_P=0; double* val_image_P=0;
    double* word_P = 0; int* row_word_P = 0; int* col_word_P = 0; double* val_word_P = 0;

    // Compute input similarities for approximate t-SNE
    // Compute asymmetric pairwise input similarities
    computeGaussianPerplexity(image, N_I, D, &row_image_P, &col_image_P, &val_image_P, perplexity, (int)(3 * perplexity));
    computeCosineGaussianPerplexity(word, N, D, &row_word_P, &col_word_P, &val_word_P, perplexity, (int)(3 * perplexity));

    // Symmetrize input similarities
    symmetrizeMatrix(&row_image_P, &col_image_P, &val_image_P, N_I);
    symmetrizeMatrix(&row_word_P, &col_word_P, &val_word_P, N);
    double sum_image_P = .0, sum_word_P = .0;
    for (int i = 0; i < row_image_P[N_I]; i++) sum_image_P += val_image_P[i];
    for (int i = 0; i < row_image_P[N_I]; i++) val_image_P[i] /= sum_image_P;
    for (int i = 0; i < row_word_P[N]; i++) sum_word_P += val_word_P[i];
    for (int i = 0; i < row_word_P[N]; i++) val_word_P[i] /= sum_word_P;

    end = clock();

    // Lie about the P-values
    for (int i = 0; i < row_image_P[N_I]; i++) val_image_P[i] *= 12.0;
    for (int i = 0; i < row_word_P[N]; i++) val_word_P[i] *= 12.0;

    // Initialize image solution (randomly)
    for (int i = N_W * no_dims; i < N * no_dims; i++) Y[i] = randn() * .0001;
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        // Compute (approximate) gradient
        computeGradient(row_image_P, col_image_P, val_image_P, Y + (N_W * no_dims), N_I, no_dims, dYI, theta);
        computeGradient(row_word_P, col_word_P, val_word_P, Y, N, no_dims, dY, theta);
        for (int i = N_W * no_dims; i < N * no_dims; i++) dY[i] = weight * dY[i] + (1 - weight) * dYI[i - (N_W * no_dims)];
        ...
        if (iter > 0 && (iter % 50 == 0 || iter == max_iter - 1)) {
            ...
            double C = .0;
            C = evaluateError(row_word_P, col_word_P, val_word_P, Y, N, theta);  // doing approximate computation here!
            if (iter == 0)
                printf("Iteration %d: error is %f\n", iter + 1, C);
            else {
    ...
    free(uY);
    free(gains);
    free(row_image_P); row_image_P = NULL;
    free(col_image_P); col_image_P = NULL;
    free(val_image_P); val_image_P = NULL;
    free(row_word_P); row_word_P = NULL;
    free(col_word_P); col_word_P = NULL;
    free(val_word_P); val_word_P = NULL;
    printf("Fitting performed in %4.2f seconds.\n", total_time);
}
...
// Compute input similarities with a fixed perplexity using ball trees (this function allocates memory another function should free)
void TSNE::computeCosineGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K) {
