...
public:
    ...
    void run(double* image, double* word, double* word_solution, int N_I, int N_W, int D, double* Y, int no_dims, double perplexity, double theta, int max_iter);
...
private:
    ...
    void computeCosineGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K);
    double randn();
};
