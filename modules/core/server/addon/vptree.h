...
public:
    ...
    DataPoint(int D, int ind, double* x, DataType type) {
        _D = D;
        _ind = ind;
        _x = (double*) malloc(_D * sizeof(double));
        for (int d = 0; d < _D; d++) _x[d] = x[d];
...
double cosine_distance(const DataPoint &t1, const DataPoint &t2) {
    ...
    dd = dd / sqrt(norm1*norm2);
    return 1.0 - dd;
}

double unify_distance(const DataPoint &t1, const DataPoint &t2) {
    if (t1.type() == none && t2.type() == none) {
        return euclidean_distance(t1, t2);
    }
    if (t1.type() == image && t2.type() == image) {
        return euclidean_distance(t1, t2);
    }
    if (t1.type() == word && t2.type() == word) {
        return cosine_distance(t1, t2);
    }
    if (t1.type() == caption && t2.type() == caption) {
        return cosine_distance(t1, t2);
    }
    if (t1.type() == caption_word && t2.type() == word_caption) {
        return cosine_distance(t1, t2);
    }
    if (t1.type() == word_caption && t2.type() == caption_word) {
        return cosine_distance(t1, t2);
    }
    return 1.0;
}
